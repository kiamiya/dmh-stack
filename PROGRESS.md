...
| S38-1 | CR (17/09) — bug import "0 contact créé" : **cause trouvée, bien plus large que l'import** — régression du correctif 018 dans `run_automation_rules()` (030/035/040 ont réintroduit `new.stage_id`) : toute insertion contacts/companies/prospects/tasks échoue en production depuis le 07/09 | 🔄 code + tests verts (migration 044 + garde-fou de non-régression + affichage du vrai message d'erreur à l'import) — **migration 044 appliquée et vérifiée en production le 2026-09-23** (confirmation Loïc) — ✅ tests fonctionnels 1-6 passés en prod (exécutés par Claude à la demande de Loïc, voir `TESTING.md`) |
| S38-2 | CR (17/09) — import : validation du format email + correction inline des valeurs en erreur | ✅ fait côté code (aucune migration) — en attente de validation navigateur, voir `TESTING.md` |
| S38-3 | CR (17/09) — import : politique de conflit (ignorer / compléter les champs vides / écraser), choix global par import (modèle HubSpot) | ✅ fait côté code (aucune migration) — validation navigateur reportée en fin de lot |
| S38-4 | CR (17/09) — import en page plein écran (`/import/contacts`, `/import/companies`) : modèle CSV téléchargeable, indicateurs de mapping (✓/○), distinction propriétés contact / entreprise | ✅ fait côté code (aucune migration) — validation navigateur reportée en fin de lot |
| S38-5 | CR (17/09) — base juridique RGPD à l'import de contacts, défaut "intérêt légitime — prospect" (B2B) | ✅ fait — **migration 045 appliquée et vérifiée en production le 2026-09-23** — validation navigateur reportée en fin de lot |
| S38-6 | CR (17/09) — les 8 champs de la fiche de prospection (S37) deviennent des champs système communs à tous les clients (options "Offres concernées"/"Grille de qualification" surchargeables par client) | ✅ fait — **migration 046 appliquée et vérifiée en production le 2026-09-23** (8 champs système, 2 valeurs S37 reprises, 0 perte) — bouton S37 retiré — validation navigateur reportée en fin de lot |
| S38-7 | CR (17/09) — édition d'un champ existant (libellé, options : ajouter/renommer/supprimer/réordonner), report sur les valeurs déjà saisies | ✅ fait côté code (aucune migration, s'appuie sur la 046) — validation navigateur reportée en fin de lot |
| S38-8 | CR (17/09) — fiche entreprise en 3 colonnes façon HubSpot (gauche : infos clés + actions rapides note/email/appel/tâche/réunion ; centre : historique ; droite : connexions) | ✅ fait côté code (aucune migration) — validation navigateur reportée en fin de lot |
...
**S38-1 — cause racine** : en production, aucune entreprise ni aucun
contact créé depuis le 03/09 (lecture `supabase db query --linked`),
alors que les champs personnalisés de la démo (`effectif_estime` 09:06,
`secteur_d_activite` 09:10, le 18/09) ont bien été créés → le wizard S36
marche, chaque ligne échouait à l'écriture. En relisant les migrations :
018 avait corrigé `record "new" has no field "stage_id"` (référence à
`new.stage_id` dans la requête de sélection des règles, partagée par
tous les triggers alors que seule `deals` a cette colonne), mais 030
(appliquée le 07/09), 035 et 040 ont été réécrites à partir de la
version 017 et ont réintroduit le bug. Impact : création manuelle de
contact/entreprise, import CSV, création de tâche (dont S34-2 "tâche de
relance") et de prospect — tous cassés depuis le 07/09. L'hypothèse du
CR ("domaine test des emails") était fausse : le code d'import ne valide
même pas le format email (sujet S38-2).

Corrigé :
- `supabase/migrations/044_fix_automation_stage_field_access_regression.sql` : version 040 + schéma 018 (`v_stage_changed`/`v_stage_id_text` calculés en plpgsql sous IF).
- `apps/crm/src/lib/automationMigrations.test.ts` : garde-fou qui lit la DERNIÈRE définition de `run_automation_rules()` dans les migrations et échoue si `new/old.stage_id` apparaît hors de la branche opportunité (vérifié : échoue sans 044, passe avec).
- `lib/importErrorSummary.ts` (+ test) branché dans `ImportEntitiesDialog` : les erreurs d'écriture ne sont plus fondues dans "N ligne(s) ignorée(s)" ; la fenêtre reste ouverte avec le vrai message regroupé par cause et les lignes concernées.
...
**Tests S38-1 exécutés par Claude** (demande de Loïc) : 6/6 ✅ en prod sur le client de test, données nettoyées. Détail dans `TESTING.md`.
...
**S38-2** : `isValidEmail` extrait de `lib/contactForm.ts` (même règle que le formulaire manuel, `x@y.z` sans espace). `planContactImport` écarte désormais une ligne à l'email mal formé avec `invalidEmail: { rowIndex, value }` (un email invalide n'entre plus dans la détection de doublons). Nouveau `lib/importRowCorrection.ts` (`applyCellCorrection`, pur, sans mutation) et composant `InvalidEmailCorrectionRow` : à l'étape récapitulatif, chaque email invalide a un champ de correction (bordure rouge tant que le format est faux, bouton "Corriger" désactivé) et un bouton "Importer sans email" — le plan est recalculé à chaque correction. Rien n'est corrigé automatiquement (Delphine : la qualité de la donnée reste de la responsabilité de l'utilisateur). Tests : +5 (crm 646), `pnpm typecheck`/`pnpm test` racine verts.

**Décisions Loïc (2026-09-23)** : (1) « fais toutes les tâches et je ferai les tests à la fin » — validation navigateur de S38-2 à S38-11 **reportée explicitement** en une session unique en fin de lot (dérogation explicite à la règle 3 pour ce lot) ; (2) migrations du lot S38 à appliquer en production **au fil de l'eau**, dès que leurs tests sont verts (autorisation explicite couvrant les migrations S38 uniquement).

**S38-3** : `lib/importConflict.ts` (pur) — `ImportConflictPolicy` (`skip`/`fill_empty`/`overwrite`), `buildConflictPatch` (une valeur vide du fichier n'efface jamais l'existant, patch limité aux clés réellement modifiées), `shouldWriteCustomFieldValue`. Les plans d'import (`planContactImport`/`planCompanyImport`) gagnent un paramètre `conflictPolicy` (défaut `skip` = comportement inchangé) et une liste `toUpdate` ; un doublon **interne au fichier** reste toujours écarté (nouveau libellé "en double dans le fichier", distinct de "déjà utilisé/existante" en base). Service : `updateExistingContacts`/`updateExistingCompanies` + `listContactsForImportConflict`/`listCompaniesForImportConflict` (`services/entityImport.ts`) — jamais de changement de l'entreprise de rattachement d'un contact ni du nom d'une entreprise ; champs personnalisés soumis à la même politique. UI : choix radio à l'étape récapitulatif, compteur "N à créer · M à mettre à jour", fiches existantes chargées dès la sortie de l'étape de correspondance. Tests : +15 (crm 661), racine verte.

**S38-4** : `components/ImportEntitiesDialog.tsx` déplacé en `pages/Import.tsx` (`ImportPage`, route `/import/:entity`), toute la logique S36/S38-1/2/3 conservée telle quelle ; les boutons "Importer" de Prospects (bascule Contacts) et `EntreprisesPanel` naviguent vers la page, retour automatique vers `/` ou `/?view=companies` après un import sans erreur. Nouveau `lib/importFields.ts` (pur) : champs standards groupés par propriétés contact/entreprise, `buildImportTemplateCsv` (en-têtes = libellés, ligne d'exemple — test : chaque en-tête du modèle est bien auto-détecté au mapping), `describeImportColumns` (statut de chaque colonne du fichier). Mapping en 2 panneaux : champs groupés avec ✓/○ (○ rouge si obligatoire non associé), tableau des colonnes du fichier avec exemple de valeur et statut. Page Aide mise à jour (import plein écran, modèle, correction des emails, politique de conflit). Tests : +5 (crm 666), build Vite OK, racine verte.

**S38-5** : migration `045_contact_legal_basis.sql` (enum `contact_legal_basis`, 6 valeurs reprises de HubSpot ; colonne `contacts.legal_basis` nullable, **sans rétro-remplissage** — on n'invente pas la base juridique des contacts existants). Type `ContactLegalBasis` dans `@dmh/types`, `lib/legalBasis.ts` (options, défaut `legitimate_interest_prospect`, libellé, parse). Import de contacts : sélecteur obligatoire au récapitulatif (défaut intérêt légitime prospect), appliqué aux contacts créés et aux contacts existants **seulement s'ils n'en ont pas encore** (une base juridique déjà retenue n'est jamais écrasée, quelle que soit la politique de conflit). Fiche contact : base juridique éditable ("Non renseignée" par défaut). La saisie manuelle d'un contact ne pose pas de base juridique (hors périmètre du CR, décision par défaut du 2026-09-23). Documentation HubSpot transmise par Delphine (Discord) non reçue — options calquées sur l'écran HubSpot décrit dans le CR. Tests : +6 (crm 672), racine verte. **Migration 045 appliquée en production le 2026-09-23** (autorisation lot S38), vérifiée (`information_schema`).

**S38-6** : choix d'implémentation — extension de `custom_field_definitions` (S9) plutôt que 8 colonnes en dur : un champ système = `client_id null` + `is_system` (contrainte de cohérence, index unique partiel sur `(entity_type, field_key)`), protégé par trigger (suppression et changement de structure interdits ; libellé/options restent éditables). Les valeurs restent cloisonnées par client. Nouvelle table `custom_field_client_options` (surcharge d'options par client, RLS 3 policies standard) + policy `system_definitions_read`. Migration `046` : insère les 8 champs système (mêmes clés que S37, dont `besoins_et_angle_accroche` — clé vérifiée contre le gabarit S37 avant application), puis fusionne les définitions S37 par client (valeurs repointées, options conservées en surcharge si différentes, définitions supprimées). Vérifié en production : 15 définitions dont 8 système, 7 valeurs dont 2 sur champs système, 0 perte. Code : `lib/customFieldScope.ts` (`resolveDefinitionsForClient` : système d'abord, puis champs du client, **jamais ceux d'un autre client**, options surchargées), `useFieldDefinitions(entityType, clientId?)`, `listClientFieldOptions`. **Défaut existant corrigé au passage** : `CustomFieldsCard` affichait les champs personnalisés de TOUS les clients sur chaque fiche (aucun filtre par client) — désormais 2 cartes "Fiche de prospection" (système) + "Champs personnalisés" (du client). Import : l'agent reçoit les champs système + ceux du client ; une clé déjà prise par un champ système est réutilisée, jamais dupliquée. Paramétrage : colonne "Portée" (Système / nom du client), filtrage par client choisi, bouton S37 et `prospectingFieldsTemplate` (lib + service + tests) supprimés. Le champ "offre" était déjà côté entreprise. Tests : crm 668 (−8 S37, +4), racine verte, build OK.

**S38-7** : bouton "Modifier" sur chaque ligne du tableau des champs (`/settings/custom-fields`) → `EditFieldDefinitionDialog`. Champ personnalisé : libellé + options sur la définition. Champ système : libellé commun non modifiable, options modifiées **pour le client choisi sur la page uniquement** (surcharge `custom_field_client_options`, S38-6) — sans client choisi, l'édition des options est bloquée avec un message. Pur : `lib/fieldOptionsEdit.ts` (brouillons d'options avec suivi de l'option d'origine → renommages/suppressions/ajouts, validation vide/doublon insensible à la casse, `migrateFieldValue` pour liste et choix multiples). Service `services/fieldDefinitionEdit.ts` : écrit la définition ou la surcharge, puis **reporte renommages/suppressions sur les valeurs déjà saisies** (du seul client concerné pour un champ système) — une option supprimée disparaît des fiches, avec avertissement explicite dans la fenêtre avant d'enregistrer. Le tableau utilise désormais les options résolues du client choisi. Pas de suppression d'un champ entier (non demandé). Tests : +11 (crm 679), racine verte.

**S38-8** : `pages/CompanyDetail.tsx` réorganisé en 3 colonnes (`xl:` ; empilées en dessous). Gauche : carte "En bref" (secteur, effectif, CA, ville, site) + **5 actions rapides** — Note et Appel ouvrent un compositeur dans la colonne centrale (choix du contact si l'entreprise a plusieurs prospects ; désactivées avec explication si aucun prospect : une interaction se rattache à un prospect en base, `interactions.prospect_id not null`), Email liste les contacts liés avec `mailto:`, Tâche ouvre `AddTaskDialog` pré-rempli (client + entreprise), Réunion ouvre `AddCalendarEventDialog` pré-rempli (désactivée sans calendrier connecté) ; puis Informations, Données Pappers, Fiche de prospection / Champs personnalisés. Centre : **historique unifié** (`lib/companyTimeline.ts`, pur : interactions + changements de statut de TOUS les prospects de l'entreprise + rendez-vous liés, plus récents d'abord, filtres Tout/Échanges/Statuts/Rendez-vous), alimenté par `useCompanyTimeline` (`listInteractionsForProspects`/`listStatusHistoryForProspects`, requêtes `in`). Droite : Contacts, Opportunités (désormais cliquables), Groupe, Tâches, Liste assignée, Rendez-vous. Nouveau service `createCallLog` (interaction `call`/`phone`). `AddTaskDialog`/`AddCalendarEventDialog` : prop `defaults` facultative (rétro-compatible). `QuickMeetingDialog` monté seulement à l'ouverture (`useUpcomingCalendarEvents` interroge Google/Microsoft dès son montage — évite un appel externe à chaque ouverture de fiche). Tests : +4 (crm 683), build OK, racine verte.

**S38-9** : bouton "Personnaliser la fiche" (en-tête de la fiche entreprise) → `CompanyLayoutDialog` : pour le client de l'entreprise, chaque bloc standard (12 : En bref & actions rapides, Informations, Données Pappers, Fiche de prospection, Champs personnalisés, Historique, Contacts, Opportunités, Groupe, Tâches, Liste assignée, Rendez-vous) se masque, monte/descend, change de colonne ; ajout/suppression de **blocs personnalisés** (titre + champs choisis parmi les champs système et ceux du client — ex. "Incidents substances toxiques") ; "Revenir à l'affichage par défaut". Enregistré **par client** (toutes ses fiches entreprise). Migration `047_company_layouts.sql` (une ligne par client, `layout jsonb`, `updated_by`, RLS 3 policies standard). Pur : `lib/companyLayout.ts` — défaut = disposition S38-8, `normalizeCompanyLayout` (JSON invalide/ancien → défaut ; blocs inconnus/doublons ignorés ; tout bloc standard absent réinséré visible dans sa colonne par défaut — une fiche ne perd jamais un bloc), opérations immuables, `fieldKeysInCustomBlocks` (un champ placé dans un bloc personnalisé visible n'est pas répété dans les cartes génériques). `CustomFieldsCard` : props `section` / `onlyFieldKeys` + `title` / `excludeFieldKeys`. Si l'historique est masqué, le compositeur note/appel/email s'affiche sous les actions rapides. Services `companyLayouts.ts` + hook `useCompanyLayout` (repli sur le défaut en cas d'erreur de lecture). Tests : +11 (crm 694), build OK, racine verte. **Migration 047 appliquée en production le 2026-09-23** (autorisation lot S38), vérifiée (RLS active, 3 policies).

**S38-10** (CR : "étudier la faisabilité" → faisable, implémenté) : migration `048_automation_prospect_status_trigger.sql` — déclencheur `status_changed` (prospect uniquement, cible optionnelle `trigger_config.to_status`), trigger `prospects_automation` étendu à `update of status` (pas à toute mise à jour), `run_automation_rules()` réécrite à partir de la 044 avec le même schéma de protection (colonnes propres à une table lues uniquement dans une branche par entité avant la boucle). Corrigé au passage : une tâche créée par automatisation depuis un prospect n'était rattachée ni au contact ni à l'entreprise — désormais les deux ; `action_config.task_type` repris. **Garde-fou généralisé** (`automationMigrations.test.ts`) : toute lecture de `new/old.stage_id|status|contact_id|company_id` doit être dans la section par entité, jamais dans la requête des règles — vérifié qu'il passe sur 044 et 048 et échoue sur 040 (version fautive). UI Automatisations : déclencheur "Au changement de statut" (prospect), sélecteur de statut cible, type de tâche (Appel/Email/RDV/Donnée) sur l'action "Créer une tâche", résumés de règle précisant le statut cible et le type. **Test de fumée en production** après application (règle réelle sur le client de test puis supprimée) : insertions contact/entreprise/prospect/tâche OK ; statut → `enriched_pappers` : aucune tâche ; statut → `enriched_contact` : 1 tâche `call`, origine automatisation, échéance J+1, rattachée au contact et à l'entreprise ; mise à jour sans changement de statut : aucune tâche ; opportunité déplacée d'étape OK ; 0 donnée restante. Pour l'activer réellement : créer la règle dans Automatisations (entité Prospect, "Au changement de statut" → "Contact enrichi", action Créer une tâche type Appel). Séquences de tâches Lemlist (LinkedIn/email/appel) toujours en attente de William (S38-N). Tests : +4 (crm 697), racine verte.

**S38-11** : le CR ne listant pas les écrans concernés, audit du code contre les règles du design "Relais" déjà documentées (S29/S31 : cartes/figures/boutons non arrondis et sans fond — "line drawings" ; icônes Lucide trait 1.5 ; jetons de thème). `DesignSync` non utilisé (réservé à la commande `/design-sync` lancée par l'utilisateur). Écarts réels corrigés (19 fichiers, classes CSS uniquement) : (1) cartes Kanban `ProspectCard`/`OpportunityCard` encore remplies + ombrées (`bg-card shadow-sm` — même écart que celui corrigé sur `Card` en S31, ces cartes ne passent pas par `Card`) → au trait ; (2) onglet actif `ui/tabs` ombré → sans ombre ; (3) `rounded` littéral (4 px, contourne le jeton `--radius: 0`) sur les boutons de bascule Liste/Kanban, Contacts/Entreprises, Statique/Dynamique, dossiers de Segments, Liste/Calendrier, menus "...", barre de complétude, pastilles de calendrier → coins carrés (0 restant) ; (4) couleurs codées en dur → jetons (`destructive`, `warning`, `success`) : alerte "stagnant" (Kanban, Prospects, dialogues de doublon), bouton de suppression d'un contact, badge de notifications, pastilles d'état des Intégrations, tendance KPI du Dashboard. Vérifié conforme, non modifié : toutes les icônes Lucide ont `strokeWidth={1.5}`. **Laissés volontairement** : ombres des éléments flottants (fenêtres, menus, notifications, panneau latéral — élévation, pas des cartes), palette du graphique `StatusBarList` (couleurs de données), variantes du composant `Badge` (pastilles colorées avec variantes sombres explicites — pas de règle documentée contraire ; à trancher avec la maquette si Loïc le souhaite), `rounded-full` des avatars/pastilles/chips (forme de puce, déjà présente dans les filtres rapides issus du mockup S35), `bg-white` de la Sidebar (transparences blanches sur fond sombre, S28/S29). Typecheck/tests (crm 697)/build/racine verts.
