...
export interface ProspectDetailContentProps {
  id: string;
  /** Emplacement libre en tête de fiche — lien "Retour" pour la page pleine largeur, bouton de fermeture pour le panneau latéral. */
  headerSlot?: ReactNode;
}

/**
 * Contenu de la fiche prospect, indépendant de son mode d'affichage —
 * réutilisé par `ProspectDetailPage` (navigation classique, lien direct)
 * et `ProspectDetailPanel` (panneau latéral superposé, navigation interne
 * à l'app). Toute la logique (hooks, mutations) vit ici une seule fois.
 */
export function ProspectDetailContent({ id, headerSlot }: ProspectDetailContentProps) {
  const {
    prospect,
    message,
    loading,
