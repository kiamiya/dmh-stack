...
describe("buildGoogleConnectUrl", () => {
  it("pointe vers la bonne Edge Function de callback avec le staffId en state", () => {
    const url = new URL(buildGoogleConnectUrl(config, "staff-42"));
    expect(url.searchParams.get("redirect_uri")).toBe(
      "https://example.supabase.co/functions/v1/google-calendar-oauth-callback",
    );
    expect(url.searchParams.get("state")).toBe("staff-42");
...
describe("buildMicrosoftConnectUrl", () => {
  it("pointe vers la bonne Edge Function de callback avec le bon tenant", () => {
    const url = new URL(buildMicrosoftConnectUrl(config, "staff-42"));
