...
const folders = [{ id: "f1", name: "Prospection" }];

const LIST_META = { created_by: null, updated_at: "2026-01-01", deleted_at: null, folder_id: null as string | null };

const contactLists = [
  { id: "cl-static", client_id: "c1", name: "Décideurs métallurgie", rules: null, created_at: "2026-01-01", ...LIST_META, folder_id: "f1" },
];
...
describe("computeListOverviewRows", () => {
  it("calcule un effectif réel pour une liste dynamique via matchesRuleGroups", () => {
    const rows = computeListOverviewRows(contactLists, companyLists, opportunityLists, clients, folders, contacts, companies, opportunities, new Map());
  ...
  it("utilise les vrais ids de membres pour une liste statique", () => {
    ...
    const rows = computeListOverviewRows(contactLists, companyLists, opportunityLists, clients, folders, contacts, companies, opportunities, staticIds);
  ...
  it("résout le nom du dossier pour une liste classée", () => {
    const rows = computeListOverviewRows(contactLists, companyLists, opportunityLists, clients, folders, contacts, companies, opportunities, new Map());
    expect(rows.find((r) => r.id === "cl-static")).toMatchObject({ folderId: "f1", folderName: "Prospection" });
  });

  it("retourne folderId/folderName null pour une liste non classée", () => {
    const rows = computeListOverviewRows(contactLists, companyLists, opportunityLists, clients, folders, contacts, companies, opportunities, new Map());
    expect(rows.find((r) => r.id === "col-dynamic")).toMatchObject({ folderId: null, folderName: null });
  });
});
