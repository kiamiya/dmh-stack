-- ============================================================
-- DMH & Associés — S33 (revue dev CRM 08/09/2026) : ajoute l'opérateur
-- "n'est pas renseigné" (inconnu), symétrique de l'opérateur "is_set"
-- déjà existant — demande du CR ("chaque type de propriété doit avoir
-- ses propres opérateurs de filtre (est, n'est pas, avant, après,
-- connu/inconnu)"). Les segments/listes dynamiques stockent leurs
-- règles en JSONB (pas de contrainte SQL à mettre à jour) ; seule la
-- table `automation_conditions` a une contrainte CHECK sur `operator`.
--
-- Volontairement PAS touché dans cette migration : la comparaison
-- numérique de `gt`/`lt` dans `run_automation_rules()` (`::numeric`)
-- lève une exception sur une valeur non numérique (ex. une date) —
-- bug réel identifié, mais cette fonction est déjà en production et
-- sensible (voir migration 030) ; corrigé côté client
-- (`lib/segmentEvaluator.ts`, pas de risque de casser un trigger
-- live), la correction côté trigger est laissée pour une session
-- dédiée avec son propre protocole de test manuel.
-- ============================================================
...
begin
  for r in
    ...
    where conrelid = 'automation_conditions'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike '%operator%'
  loop
    execute format('alter table automation_conditions drop constraint %I', r.conname);
  end loop;
end $$;

alter table automation_conditions
  add constraint automation_conditions_operator_check
  check (operator in ('eq', 'neq', 'gt', 'lt', 'contains', 'is_set', 'is_not_set'));
